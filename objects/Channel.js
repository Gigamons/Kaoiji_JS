...
function JoinChannel(token, channel = '#osu') {
  ...
  if (TokenData) {
    const PrivilegePerm = TokenData.general.Privileges;
    ...
    if (channelExists(channel)) {
      if (channelAdminOnly(channel)) {
        if (common.PrivilegeHelper.hasPrivilege(PrivilegePerm, common.Privileges.BAT)) {
          ...
          channels[info[0]].JoinedUsers.push(JoinedUsersOBJECT([TokenData.general.UserID, TokenData.general.UserName, TokenData.token]))
