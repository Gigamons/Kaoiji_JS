...
function checkToken(token){
    for(let i = 0, len = Tokens.length; i < len; i++){
        ...
        } catch (ex){ console.error(ex); return false} // There should be no exception.
    }
}
...
async function removeToken(token = null, userid = 0){
    let useToken = true;
    if(userid > 0){useToken = false;}
    else if(token === null || token === undefined) {useToken = false;}
    if(useToken){
        for (let i = 0; i < Tokens.length; i++) {
            const e = Tokens[i];
            if(e.token === token){
...
function StartTimeoutCheck(){
    setInterval(() => {
        Tokens.forEach((e, i, arr) => {
            ...
            if(e.TimeoutTimer < ms-(30*60*60) && e.general.UserID != 100)
                removeToken(e.token);
        });
    }, 5000);
}
