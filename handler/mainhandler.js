...
async function handlePackets (token, RawPacket) {
    ...
    require('../heavyhandler/pong')(token);
    for(let i = 0, len = PacketData.length; i < len; i++){ 
        ...
        switch (Packet.id) {
            ...
            case 1:
                ...
                require('../heavyhandler/message').sendMessageToChannel(token, Packet.data.message, Packet.data.target);
                break;
            case 2:
            ...
            case 4:
                ...
                ++Token.Tokens[TokenData[0]].ping;
                if(Token.Tokens[TokenData[0]].ping == 0){
                    await require('./private/UpdateStatistics')(token, undefined, true)
                    require('./public/requestStatusUpdate')(token);
                }
                require('./public/requestStatusUpdate')(token);
            ...
            case 85:
                ...
                for (let i = 0; i < Packet.data.length; i++) {
                    ...
                    let bot;
                    if(userid == 100) bot = true; else bot = false;
