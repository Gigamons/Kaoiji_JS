...
const login = async (res, req, packet) => {
  ...
  if (LoginData != undefined) {
    try {
      ...
      const checkAuth = await UserTools.checkAuth(userid, LoginData.password);
      ...
      try {
        ...
        latitude = Number(IP.loc.split(',')[1]);
      } catch (ex) {
        console.log(ex);
        countryId = Number(0);
        longitude = Number(0);
      ...
      switch (ex) {
        case 'username':
          loginFailed(res, LoginData.username);
          res.end();
          return;
          break;
        case 'password':
...
const loginFailed = (res, Username = '') => {
  const writer = new OsuPacket.Bancho.Writer;
  writer.LoginReply(-1);
  res.writeHead(200, {
    'cho-token': 'Nope >:(',
  ...
  console.log('(A Wild ' + Username + ' Failed to login.) He said: Fuck you bancho!');
}
